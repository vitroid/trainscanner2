...
class Peak:
    def __init__(self, xy, value):
        ...
        self.history = [(xy[0], xy[1], value)]

    def predict(self):
        print(f"Predict from {self.mean=}")
    ...
    def update(self, xy, value, missed=False):
        ...
        self.history.append((xy[0], xy[1], value))
        self.mean = new_mean
    ...
    def missed(self):
        # 予測値でupdateする(?)
        xy = self.predicted
        self.update((int(xy[0]), int(xy[1])), value=0, missed=True)
...
for motion in motions:
    ...
    for p in persistents.keys():
        persistents[p].predict()
    ...
    points = np.array(list(maxima.keys()))
    print(f"{points=}")

    unassigned = set([tuple(xy) for xy in maxima.keys()])
    missed = set(persistents.keys())

    if len(points) > 0:
        for p in persistents.keys():
            xy, d = persistents[p].closest(points)
            if xy is not None and d <= 3:
                xy = tuple(xy)
                persistents[p].update(xy=xy, value=maxima[xy])
                unassigned -= {xy}
                missed -= {p}

    for p in missed:
        missed_duration = persistents[p].missed()
        if missed_duration > 3:
            print(f"long missed {p=} {missed_duration=}")
            del persistents[p]

    for xy in unassigned:
        xy = tuple(xy)
        persistents[n_persistent] = Peak(xy=xy, value=maxima[xy])
        n_persistent += 1
    ...
    for p in persistents.keys():
        print(
            f"{p=}: {persistents[p].missed_duration=} {[(x,y) for x,y,v in persistents[p].history]}"
        )
    print()
